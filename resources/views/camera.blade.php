@extends('layouts.app') @section('title', 'Absensi Kamera') @section('content')

<style>
    /* Custom Styles for Attendance Camera */
    .attendance-wrapper {
        min-height: 100vh;
        background: linear-gradient(135deg, #f6f6f6 0%, #c1bdf8 100%);
        padding: 1.5rem;
    }
    ...
    .btn-modal {
        ...
        border: none;
        cursor: pointer;
        background: linear-gradient(135deg, #3452dc 0%, #2b61c5 100%);
        color: white;
        transition: all 0.3s ease;
    ...
    <div class="attendance-container">
        ...
        <div class="header-card">
            ...
            <p class="header-subtitle">
                Pastikan kamera dan GPS aktif untuk melakukan absensi
            </p>
        </div>
        ...
        <div class="status-card">
            <div class="status-header">
                ...
                <div class="status-content">
                    ...
                    <p id="schedule-text" class="status-text">
                        Memeriksa jadwal...
                    </p>
                </div>
            </div>
        ...
        <div class="gps-card">
            <div class="gps-header">
                ...
                <div class="gps-content">
                    ...
                    <div id="gps-status" class="gps-status-badge">
                        Mendeteksi GPS...
                    </div>
            ...
            <div
                ...
                style="display: none"
            >
                <label class="location-selector-label"
                    >Lokasi Absensi Terdekat</label
                >
            ...
            <div class="camera-wrapper">
                ...
                <div class="camera-controls">
                    ...
                    <button
                        ...
                        class="btn-camera btn-camera-secondary"
                        disabled
                    >
                        <svg
                            style="width: 20px; height: 20px"
                ...
                >
                    <div class="location-content">
                        ...
                        <div class="location-meta">
                            <span class="location-radius-badge"
                                >Radius {{ $loc->radius }}m</span
                            >
                        </div>
                    </div>
                </div>
...
<script>
    class AttendanceCamera {
        constructor() {
            ...
            this.nearestLocationSelect =
                document.getElementById("nearest-location");
        ...
        loadLocations() {
            const locationElements =
                document.querySelectorAll("[data-location-id]");
        ...
        setupEventListeners() {
            ...
            this.nearestLocationSelect.addEventListener("change", (e) => {
                this.selectedLocation = this.attendanceLocations.find(
        ...
        updateLocationDistances() {
            ...
            const locationsWithDistance = this.attendanceLocations
                .map((loc) => {
            ...
            locationsWithDistance.forEach((loc) => {
                const distanceEl = loc.element.querySelector(
                    ".location-distance-text"
                );
                if (distanceEl) {
                    ...
                    distanceEl.querySelector(".distance-value").textContent =
                        distanceText;
            ...
            this.nearestLocationSelect.innerHTML =
                '<option value="">Pilih lokasi...</option>';
        ...
        async requestPermissions() {
            try {
                ...
                this.showModal(
                    "Error",
                    "Akses kamera diperlukan untuk absensi.",
                    "error"
                );
                return false;
            }
        ...
        onPositionSuccess(position) {
            ...
            if (accuracy <= 10) {
                ...
                textEl.textContent =
                    "GPS Cukup Akurat - Dapat digunakan untuk absensi";
                statusEl.style.background =
                    "linear-gradient(135deg, #f59e0b 0%, #d97706 100%)";
                statusEl.style.color = "white";
            } else {
                textEl.textContent =
                    "GPS Kurang Akurat - Pindah ke lokasi terbuka";
                statusEl.style.background =
                ...
                this.showModal(
                    ...
                    "Akurasi GPS terlalu rendah (" +
                        accuracy +
                        " m). Pindah ke lokasi terbuka untuk hasil lebih baik.",
                    "warning"
                );
            }
        ...
        showModal(title, message, type = "info") {
            ...
            const iconHTML = {
                success:
                    '<svg style="width: 28px; height: 28px;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>',
        ...
        async checkSchedule() {
            try {
                const response = await fetch(
                    '{{ route("attendance.check-schedule") }}',
                    {
                        method: "GET",
                        headers: {
                ...
                if (!data.success) {
                    this.showModal(
                        "Peringatan",
                        data.message || "Tidak ada jadwal untuk hari ini.",
                        "warning"
                    );
                    this.scheduleText.textContent =
                        data.message || "Tidak ada jadwal untuk hari ini.";
                ...
                if (data.is_holiday) {
                    this.scheduleText.textContent =
                        "Hari ini adalah libur nasional / akhir pekan.";
                    this.scheduleType.textContent = "-";
                    this.startTime.textContent = "-";
                    ...
                    this.showModal(
                        "Info",
                        "Hari ini libur. Absensi tidak diperlukan.",
                        "info"
                    );
                    this.submitBtn.disabled = true;
                    return;
                ...
                if (data.approval && data.approval.status === "approved") {
                    this.scheduleText.textContent =
                        "Absensi otomatis berdasarkan pengajuan.";
                    this.scheduleType.textContent = "-";
                    this.startTime.textContent = "-";
                    this.attendanceStatus.textContent =
                        data.approval.type.toUpperCase();
                    this.attendanceStatus.style.color = "#8b5cf6";
                    this.showModal(
                        "Info",
                        "Pengajuan anda sudah disetujui. Absensi otomatis tercatat.",
                        "info"
                    );
                    this.submitBtn.disabled = true;
                    return;
                ...
                if (data.has_attendance) {
                    ...
                    this.startTime.textContent = "-";
                    this.attendanceStatus.textContent =
                        data.attendance.status || "SUDAH ABSEN";
                ...
                this.scheduleText.textContent =
                    "Jadwal ditemukan untuk hari ini. Silakan lakukan absensi.";
                this.scheduleType.textContent = data.schedule.is_fulltime
                    ? "Full-Time"
                    : "Shift";
                ...
                this.showModal(
                    "Error",
                    "Gagal memeriksa jadwal: " + err.message,
                    "error"
                );
                this.scheduleText.textContent =
                    "Gagal memeriksa jadwal. Hubungi admin.";
                this.scheduleType.textContent = "-";
                this.startTime.textContent = "-";
                this.attendanceStatus.textContent = "-";
                this.submitBtn.disabled = true;
        ...
        async submitAttendance() {
            ...
            if (distance > this.selectedLocation.radius) {
                return this.showModal(
                    "Error",
                    `Anda berada di luar jangkauan lokasi ${
                        this.selectedLocation.name
                    }. Jarak: ${Math.round(distance)}m, Radius: ${
                        this.selectedLocation.radius
                    }m`,
                    "error"
                );
            }
            ...
            this.submitBtn.disabled = true;
            this.submitBtn.querySelector(".loading-spinner").style.display =
                "inline-block";
            ...
            try {
                ...
                formData.append(
                    "latitude",
                    String(this.currentPosition.coords.latitude)
                );
                formData.append(
                    "longitude",
                    String(this.currentPosition.coords.longitude)
                );
                formData.append(
                    "accuracy",
                    String(this.currentPosition.coords.accuracy)
                );
                formData.append("location_id", this.selectedLocation.id);
                formData.append("_token", "{{ csrf_token() }}");

                const response = await fetch(
                    '{{ route("attendance.camera.submit") }}',
                    {
                        method: "POST",
                        body: formData,
                    }
                ...
                if (!data.success) {
                    ...
                    const status =
                        data.attendance?.status || data.status || "Hadir";
                    this.showModal(
                        "Sukses",
                        `Absensi berhasil di lokasi ${
                            this.selectedLocation?.name || "terpilih"
                        }! Status: ${status}`,
                        "success"
                    );
                    this.retakePhoto();
                ...
                this.showModal(
                    "Error",
                    "Terjadi error saat submit absensi: " + err.message,
                    "error"
                );
            } finally {
                this.submitBtn.disabled =
                    !this.photoBlob || !this.currentPosition || !this.schedule;
                this.submitBtn.querySelector(".loading-spinner").style.display =
                    "none";
            }
        }
