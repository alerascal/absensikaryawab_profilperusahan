...
@section('title', 'Daftar Shift - AttendPro')
...
<div class="main-content" style="padding: 24px; font-family: 'Inter', sans-serif; background: #f4f7fc;">
    ...
    <div class="attendance-panel" 
        ...
        <div style="display: flex; flex-direction: column; gap: 12px; justify-content: space-between; flex-wrap: wrap;">
            <div>
                <h1 style="font-size: 24px; font-weight: 600; color: #1f2937;">Daftar Shift</h1>
                <p style="color: #6b7280;">Kelola data shift dengan mudah</p>
            </div>
            <div>
        ...
        <!-- Alert -->
        @if(session('success'))
            <div id="alert-success" style="background: #d1fae5; color: #065f46; padding: 12px 16px; border-radius: 0.75rem; margin: 16px 0; box-shadow: 0 2px 6px rgba(0,0,0,0.1);">
                {{ session('success') }}
            </div>
        @endif
            ...
            <table style="min-width: 600px; width: 100%; border-collapse: collapse;">
                <thead style="background: linear-gradient(90deg, #7c3aed, #4f46e5); color: #fff;">
                    <tr>
                        ...
                        <th style="padding: 12px; text-align: left;">Nama Shift</th>
                        <th style="padding: 12px; text-align: center;">Jam Mulai</th>
                        <th style="padding: 12px; text-align: center;">Jam Selesai</th>
                ...
                <tbody style="background: #fff;">
                    @forelse($shifts as $key => $shift)
                        <tr style="border-bottom: 1px solid #e5e7eb;">
                            ...
                            <td style="padding: 12px;">{{ $shift->name }}</td>
                            <td style="padding: 12px; text-align: center;">{{ $shift->start_time }}</td>
                            <td style="padding: 12px; text-align: center;">{{ $shift->end_time }}</td>
                            <td style="padding: 12px; display: flex; justify-content: center; gap: 8px;">
                                ...
                                <form action="{{ route('admin.shifts.destroy', $shift->id) }}" method="POST" onsubmit="return confirmDelete(event)">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
                    ...
                    @empty
                        <tr>
                            <td colspan="5" style="text-align:center; padding:16px; color:#9ca3af;">Belum ada shift</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
