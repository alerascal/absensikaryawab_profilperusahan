...
<div class="pengajuan-container">
    <h1 class="pengajuan-title">Edit Pengajuan</h1>
    
    <form action="{{ route('admin.pengajuan.update', $pengajuan) }}" method="POST" enctype="multipart/form-data" class="pengajuan-form">
        @csrf
        @method('PUT')
        <div class="mb-4">
            <label for="title" class="pengajuan-form-label">Judul</label>
            <input type="text" name="title" id="title" class="pengajuan-form-input" value="{{ old('title', $pengajuan->title) }}" required>
            @error('title')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" class="pengajuan-form-textarea" required>{{ old('description', $pengajuan->description) }}</textarea>
            @error('description')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            @if ($pengajuan->surat_dokter)
                <p class="file-preview">File saat ini: <a href="{{ Storage::url($pengajuan->surat_dokter) }}" target="_blank">Lihat Surat Dokter</a></p>
            @endif
            @error('surat_dokter')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        ...
        <div class="mb-4">
            ...
            @if ($pengajuan->surat_izin)
                <p class="file-preview">File saat ini: <a href="{{ Storage::url($pengajuan->surat_izin) }}" target="_blank">Lihat Surat Izin</a></p>
            @endif
            @error('surat_izin')
                <div class="text-red-600 text-sm mt-1">{{ $message }}</div>
            @enderror
        ...
        <div class="flex space-x-4">
            <button type="submit" class="pengajuan-btn btn-primary">Update</button>
